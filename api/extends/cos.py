...
@Maintainer: dgos
@Des: Tencent Cloud Object Storage
"""
...
async def get_federation_token(req: Request, file_type: str):
    ...
    if not result:
        return fail(msg="Please configure Tencent Cloud object storage parameters!")
    config = {
    ...
    try:
        ...
        if not credentials:
            return fail(msg="API failure, please try again later!")
        res = {
        ...
        return success(msg="Temporary visit token", data=res)
    except Exception as e:
        print(e)
        return fail(msg="Interface failure, please try again later!")
